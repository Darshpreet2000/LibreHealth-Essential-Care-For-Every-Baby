...
class UserActivityRepository {
  ...
  UserActivityRepository(this.context, this.lock);
  Future fetchUsersMessages() async {
    try {
      Profile profile = HiveStorageRepository().getProfile();
      String response = await UserActivityClient(http.Client(), context, lock)
      ...
      for (var item in res['messageConversations']) {
        ...
        String details = await UserActivityClient(http.Client(), context, lock)
